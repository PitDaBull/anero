...
anerod_base = [builddir + "/bin/anerod", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "anerod_p2p_port", "--rpc-bind-port", "anerod_rpc_port", "--zmq-rpc-bind-port", "anerod_zmq_port", "--zmq-pub", "anerod_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "anerod_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
anerod_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/anero-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_ANERODS):
  ...
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/anerod' + str(i) + '.log', 'a+'))
