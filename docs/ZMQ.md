# The Current/Future Status of ZMQ in Anero

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Anero
...
matching supported by both Anero and ZMQ. The `format`, `context` and `event`
...
The Anero daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Anero daemon also ensures that
...
The Anero daemon will send a `txpool_add` pub exactly once for each
