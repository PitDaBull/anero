...
# Copyright (c) 2022-2024 The Anero Project
...
# target, so we don't spend time building Anero binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/anero-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,anero-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Anero build for HOST will be
