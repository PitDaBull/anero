...
if test -z "$anerod"
then
  echo "anerod not found"
  exit 1
fi
echo "Found: $anerod"

TORDIR="$DIR/anero-over-tor"
TORRC="$TORDIR/torrc"
...
"$anerod" \
  ...
  --add-priority-node 2anrnode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$anerod" status)
...
if test "$ready" = 0
then
  echo "Error starting anerod"
  tail -n 400 "$HOME/.bitanero/bitanero.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Anero\ \'" | tail -n 20
  exit 1
fi
